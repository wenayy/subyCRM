...
const SERPAPI_KEY = process.env.SERPAPI_KEY || "071f7cfa94d3016b82033cc475a212f24e72988eee2fe8790d0672ed6614b23d";
...
Return ONLY valid JSON, no markdown:
{
  ...
  try {
    ...
    if (platform === "linkedin" && out.fullName) {
      const contactWords = context.name.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(w => w.length > 1);
      const matchWords = out.fullName.toLowerCase().replace(/[^a-z0-9\s]/g, "").split(/\s+/).filter(w => w.length > 1);
      
      const hasWordOverlap = contactWords.some(cw => matchWords.some(mw => mw.includes(cw) || cw.includes(mw)));
      if (!hasWordOverlap) {
...
export const enrichmentService = {
  async enrichContact(contactId: string) {
    ...
    if (!xPlatform) {
      try {
        // Search by name + company for better precision
      ...
      // Already have X — fetch and verify to get full details
      try {
    ...
    // ── Post-enrichment classification and summary updates ────────────────────
    ...
    if (updatedContact) {
      ...
      try {
        ...
        const t = sanitizeType(classResult.type);
        const d = sanitizeDomain(classResult.domain);
